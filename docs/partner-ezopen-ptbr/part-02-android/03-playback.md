...
<!-- source: packages/crawler/docs/sdk/Android SDK/Android 回放/云存储回放.md -->
<!-- source: packages/crawler/docs/sdk/Android SDK/Android 回放/SD卡回放.md -->

Ao concluir este capítulo, você será capaz de:

- Diferenciar **playback na nuvem** (`.rec`) e **playback local (cartão SD)** no Android.
- Iniciar, pausar/parar e liberar o player para cada modo.
...
> **Escopo v1 (ADR-005):** Android documenta **nuvem** e **local (SD)** onde o SDK oficial suporta. Web documenta apenas nuvem — ver [matriz](../part-01-shared-concepts/02-glossary-capability-matrix.md).
...
1. Confirmar que o dispositivo reporta SD presente e gravações disponíveis.
2. Usar o fluxo de **playback local** documentado no AAR (busca por data, arquivo por arquivo, conforme versão).
3. **Não** reutilizar URI `.rec` de nuvem para conteúdo SD — são pipelines distintos no SDK.

Comportamento esperado para o usuário:

- Se não houver clipe no intervalo: mensagem clara (“sem gravação neste período”).
- Se a gravação estiver criptografada: solicitar senha/fluxo oficial do dispositivo (mensagem do SDK).
...
| Pausar | Usar API de pause do SDK; manter token válido em sessões longas |
| Seek / mudar clipe | Parar → nova URI ou API de seek conforme documentação |
| Sair da tela | `stop` + `release` — mesmo rigor que preview |
...
| SD não lista arquivos | Cartão ausente, não formatado, ou política de gravação desligada |
